...
class AuthProvider extends ChangeNotifier {
  ...
  Future<UserModel?> _loadCachedUser() async {
    try {
      final prefs = await SharedPreferences.getInstance();
      final userStr = prefs.getString(_userCacheKey);
      if (userStr != null) {
  ...
  Future<void> _initUser() async {
    ...
    if (user != null) {
      // Enterprise Optimization: Load cached user first for INSTANT UI
      final cachedUser = await _loadCachedUser();
      if (cachedUser != null && cachedUser.uid == user.uid) {
        _userModel = cachedUser;
        _status = AuthStatus.authenticated;
        notifyListeners();
        debugPrint("Enterprise Auth: Instant login via cache.");
      } else {
        _status = AuthStatus.authenticated;
        notifyListeners();
      }

      // Background parallel tasks: Sync logic & Silent Auth refresh
      _fetchUserModel(user.uid);
      _googleSignIn.signInSilently().catchError((e) => null);
  ...
  Future<bool> submitOwnerRequest(Map<String, dynamic> data) async {
    if (_userModel == null) return false;
    _setLoading(true);
    try {
      final requestData = {
